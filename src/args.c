...
static args_option_t* args_get_short_option(args_option_t *options,
		const char opt)
{
...
static args_option_t* args_get_long_option(args_option_t *options,
		const char* arg)
{
	arg += 2; // skip starting --
...
static int args_parse_option(args_option_t* opt,
		const char* opt_arg, int* opt_arg_was_used)
{
	char *endptr = NULL;
...
static void args_print_error(int error,
		const char* option)
{
	switch(error) {
