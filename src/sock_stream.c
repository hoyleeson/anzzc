    ...
    if (result == 0) {
        ...
        if (errno == EINPROGRESS) {
            ...
            if (n == -1 && errno != EINTR) {
                ...
                int optval;
                int optlen = 4;
                if (getsockopt(stream->sock, SOL_SOCKET, SO_ERROR, (void *)&optval,
                               (socklen_t *)&optlen) < 0)
 ...
 * @retval return a non-zero error code if failed
 */
static int stream_recv_timeout(struct netsock *nsock, void *data, int len,
                               unsigned long ms)
{
    int nready;
    fd_set rset;
 ...
 * @retval return a non-zero error code if failed
 */
static int stream_send_by_session(struct netsock *nsock, void *session,
                                  void *buf, int len)
{
    int ret;
...
static void *process_connection(void *arg)
{
    ...
    while (running) {
        ...
        if (nready <= 0)  {
            ...
            if ((pack.datalen = recv(param->sock, pack.data, nsock->args.buf_size,
                                     0)) <= 0) {
                close(param->sock);
...
static void *stream_listen_thread(void *args)
{
    ...
    stream->sock_addr.sin_addr.s_addr = htonl(
                                            INADDR_ANY); /* Any incoming interface */

    ret = bind(stream->sock, (struct sockaddr * )&stream->sock_addr,
               sizeof cli_addr);
    if (ret < 0)
        return 0;
